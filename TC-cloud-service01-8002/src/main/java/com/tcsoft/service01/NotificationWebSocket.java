...
import com.tcsoft.service01.entity.WebSocketClient;
 ...
 */
@Component
@ServerEndpoint("/notification/{merchantId}")
public class NotificationWebSocket {
    ...
    public static boolean checkSocketConnected(String merchantId){
        try {
            List<WebSocketClient> webSocketClientList = webSocketClientMap.get(merchantId);
            return webSocketClientList == null;
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException(e.getMessage());
        }
